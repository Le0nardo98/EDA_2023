...
int escreverFicheiro_meios(Meio* inicio_meios, FILE* dados_meios)
{
    if (inicio_meios == NULL)
    {
        //printf("O ficheiro nao foi lido.\n");
        perror("O ficheiro nao foi lido");
        return 1;
    }
...
int escreverFicheiro_meios_bin(Meio* inicio_meios, FILE* dados_meios)
{
    if (inicio_meios == NULL)
    {
        //printf("O ficheiro nao foi lido.\n");
    ...
    while (inicio_meios != NULL)
    {
        //fprintf(dados_meios, "%d;%s;%.2f;%.2f;%d;%s;%d\n", inicio_meios->codigo, inicio_meios->tipo, inicio_meios->bateria, inicio_meios->autonomia,
        //    inicio_meios->custo, inicio_meios->geocodigo, inicio_meios->ativo);
        fwrite(inicio_meios, 1, sizeof(inicio_meios), dados_meios);
        inicio_meios = inicio_meios->seguinte_meio;
    }
    fclose(dados_meios);
}
...
int escreverFicheiro_clientes(Cliente* inicio_clientes, FILE* dados_clientes)
{
    if (inicio_clientes == NULL)
    {
        //printf("O ficheiro nao foi lido.\n");
...
int escreverFicheiro_clientes_bin(Cliente* inicio_clientes, FILE* dados_clientes)
{
    if (inicio_clientes == NULL)
    {
        //printf("O ficheiro nao foi lido.\n");
    ...
    {
        //printf("Nao foi possivel abrir o ficheiro.\n");
        perror("Nao foi possivel abrir o ficheiro"); //mais tarde escrever para ficheiro
        return 2;
    }
    while (inicio_clientes != NULL)
    {
        //fprintf(dados_clientes, "%d;%s;%d;%d\n", inicio_clientes->codigo, inicio_clientes->nome, inicio_clientes->NIF, inicio_clientes->saldo);
        fwrite(inicio_clientes, 1, sizeof(inicio_clientes), dados_clientes);
        inicio_clientes = inicio_clientes->seguinte_cliente;
    }
    fclose(dados_clientes);
}
...
int escreverFicheiro_gestores(Gestor* inicio_gestores, FILE* dados_gestores)
{
...
int escreverFicheiro_gestores_bin(Gestor* inicio_gestores, FILE* dados_gestores)
{
    if (inicio_gestores == NULL)
    {
        //printf("O ficheiro nao foi lido.\n");
    ...
    {
        //fprintf(dados_gestores, "%d;%s;%s;%d;%s\n", inicio_gestores->codigo, inicio_gestores->nome,inicio_gestores->senha,inicio_gestores->encriptado, inicio_gestores->area_responsavel);
        fwrite(inicio_gestores, 1, sizeof(inicio_gestores), dados_gestores);
        inicio_gestores = inicio_gestores->seguinte_gestor;
    }
    fclose(dados_gestores);
}
...
// De seguida é inserido no ultimo lugar da lista dos clientes, quando o ultimo endereço  é NULL.
...
// Pequena função para encriptação da senha do gestor (o que fazer aqui?).
...
// Pequena função para desencriptação da senha do gestor (o que fazer aqui?).
...
Cliente* alterarDadosCliente(Cliente* inicio_clientes) {
    int codigo, NIF, novo_codigo, novo_NIF, escolha;
    char novo_nome[50];
...
void listarGeocodigo(Meio* inicio_meios)
{
    ...
    Meio* aux_inicio_meios = inicio_meios;
    ...
    while (aux_inicio_meios != NULL && existe ==0)
    {
        if (strcmp(verificar_geocodigo, aux_inicio_meios->geocodigo) == 0)
        {
            existe = 1;
        }
        aux_inicio_meios = aux_inicio_meios->seguinte_meio;
    }
...
int escreverFicheiro_aluguer(Aluguer* inicio_aluguer, FILE* dados_aluguer)
{
    if (inicio_aluguer == NULL)
    {
        //printf("O ficheiro nao foi lido.\n");
...
int escreverFicheiro_aluguer_bin(Aluguer* inicio_aluguer, FILE* dados_aluguer)
{
    if (inicio_aluguer == NULL)
    {
        //printf("O ficheiro nao foi lido.\n");
    ...
    {
        //fprintf(dados_aluguer, "%d;%s;%s;%s\n", inicio_aluguer->cod_comprador, inicio_aluguer->data_compra, inicio_aluguer->nome_comprador, inicio_aluguer->nome_meio_comprado);
        fwrite(inicio_aluguer, 1, sizeof(inicio_aluguer), dados_aluguer);
        inicio_aluguer = inicio_aluguer->seguinte_compra;
    }
    fclose(dados_aluguer);
}
